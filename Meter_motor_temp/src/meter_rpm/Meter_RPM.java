...
public class Meter_RPM {
    ...
    /* Deal with the arguments on the command line */
        ...
        Socket socket = new Socket(ip, port);
          ...
          final Stackoverflow so;
                so = new Stackoverflow(25, "DMOC/Motor TEMPERATURES");
   ...
   // ======== Endless loop ====================================================
     ...
     //         System.out.format("%s\n",msg);
        ...
        if (can1.id == 0xCA200000){ /* Gevcur Sends (commanded) speed */
             eng[1] = can1.get_1byte(1);   //  INVertor raw temperature
             engd[1] = (float)(eng[1]) - 40; // Subtract offset and float

           // For motor: use largest of rotor and stator temperatures
                if (can1.get_1byte(0) > can1.get_1byte(2)){
                    eng[0] = can1.get_1byte(0); 
                }
                else {
                    eng[0] = can1.get_1byte(2);
                }
             engd[0] = (float)(eng[0]) - 40; // Subtract offset and float
         }
